...
2. 在 VSCode 中的 Node 环境，**控制台是无法完全展示对象内部参数**；只需在整个**文件的最后一行打上一个断点**就可以完全显示

3. **立即执行函数**：如果前置代码调用函数时未加分号，由于 js 自动补全分号的机制，会将前面的调用和之后的立即执行函数看成是**调用函数的返回值**。( 函数调用返回一个函数再调用 )

   在声明立即执行函数之前先写分号，就无需考虑上面的代码有没有加分号的情况
...
**解决**：`Object.freeze()` 冻结对象的属性值，使其不可变；但只能冻结第一层对象的属性的值，嵌套对象的属性值仍可以修改，冻结嵌套对象的属性值，递归调用 `Object.freeze()`
